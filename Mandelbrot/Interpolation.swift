...
typealias Interpolant = (Double) -> [Double]


/// The type of interpolation to perform
///
/// - Linear: Linear interpolation
/// - Cubic: Monotonic cubic interpolation
...
/// Returns a function that performs multivarible interpolation based on input
func createInterpolant(xs: [Double], ys: [[Double]], type: InterpolationType) -> Interpolant {
    return { x in
        ...
        switch type {
        case .Linear:
            return zip(ys[i], ys[i+1]).map { a, b in x * (b - a) + a }
        case .Cubic:
            // help the compiler to infer closure type
            let (ys, c1s, c2s, c3s) = cubicInterpolation(xs: xs, ys: ys)
            let zips = c3s.enumerated().map {
                p -> IteratorSequence<Zip4Generator<IndexingIterator<Array<Double>>, IndexingIterator<Array<Double>>, IndexingIterator<Array<Double>>, IndexingIterator<Array<Double>>>> in
                let (i, _) = p
...
//! monotonic cubic interpolation (single variable)
fileprivate func cubicInterpolation(xs: [Double], ys: [[Double]]) -> ([[Double]], [[Double]], [[Double]], [[Double]]) {
    let length = xs.count

    // deal with length issues
    guard length == ys.count, length > 0 else { return ([], [], [], []) }
    guard length > 1 else { return (ys, [], [], []) }
